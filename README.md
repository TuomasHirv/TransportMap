...
- **Service days.** Separate timetables for weekday, Saturday and Sunday, including
  the after-midnight services that GTFS files under the _previous_ day at
  `24:00`–`27:00`.
...
3. Trips are grouped into _routes_ by identical stop sequence — the paper's
...
4. Stops, coordinates, names and walking transfers (with transitive closure) are
   ...
   A query then runs RAPTOR in rounds, where round _k_ is "reachable using at most
   _k_ vehicles", with local pruning, the marked-stops optimisation and the time
...
2. Run the prep script (needs `pyogrio` and `geopandas`, which are _not_ app
...
- **Memory.** Previously the process peaked at a very high memory usage. This was due to how the data was read and processed.
  After fixes the peak has dropped to 424 MB and after start up it lands at 321 MB.
...
Delling, D., Pajor, T., & Werneck, R. (2012). _Round-Based Public Transit Routing._
ALENEX 2012.
